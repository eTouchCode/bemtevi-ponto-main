<x-layout bodyClass="g-sidenav-show  bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        <!-- Navbar -->
        <x-navbars.navs.auth titlePage="{{__('Editing :name', ['name' => __('Position')])}}"></x-navbars.navs.auth>
        <!-- End Navbar -->
                        ...
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                            <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                                <h6 class="text-white mx-3">
                                    {{__('Editing :name', ['name' => __('Position')])}}
                                </h6>
                            </div>
                        </div>
                        <div class="card-body px-5 pb-5">
                                    ...
                                    <div class="form-group col-6">
                                        <div class="input-group input-group-dynamic mt-3 is-filled">
                                            ...
                                            <select name="salary_id" class="form-control">
                                                <option {{empty($position->salary) ? '' : 'selected'}} value="">
                                                    {{__("Select")}}
                                                </option>
                                                @foreach ($salaries as $salary)
                                                    <option {{$salary->id != $position->salary->id ? "" : "selected"}}
                                        ...
                                        <div class="input-group input-group-dynamic mt-3 is-filled">
                                            ...
                                            <select name="workshift_id" class="form-control">
                                                ...
                                                @foreach ($workshifts as $workshift)
                                                    <option {{$workshift->id != $position->workshift->id ? "" : "selected"}}
                            ...
                            <div class="row justify-content-between">
                                <div class="col-3 pt-5 text-muted">
                                    <small>{{__("Created At")}}
                                        {{\Carbon\Carbon::parse($position->created_at)->format('d/m/Y H:i:s')}}
                                    </small>
                                    <br>
                                    <small>{{__("Last Updated")}}
                                        {{\Carbon\Carbon::parse($position->updated_at)->format('d/m/Y H:i:s')}}
                                    </small>
                                </div>
                            </div>
                        </div>
